"""Public report API: builds the markdown report and renders it to HTML.

Rendering is dispatched by markdown prefix to one of three renderers, each
in its own module:
- default: plain markdown -> HTML (this module)
- report_hybrid_dashboard: '# Hybrid Quant Daily Stock Report'
- report_hybrid_interactive: '# Hybrid 量化每日選股報告' / '# Hybrid 台股每日分析報告'
"""
...
def save_report_html(report_dir: Path, report_date: date, content: str) -> Path:
    ...
    title = markdown_title(content) or f"Stock Signals - {report_date.isoformat()}"
...
def markdown_to_html(markdown: str, title: str) -> str:
    ...
    body_lines = render_markdown_body(markdown)
