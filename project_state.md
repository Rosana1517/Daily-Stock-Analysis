...
已完成(璞玉整合輪):
- **HiStock 重試機制拖垮排程、當天沒發 LINE 的真事故 + 斷路器修正(2026-08-27)**：使用者回報「今天該排程沒有執行並觸發 LINE」。用 `gh run list`/`gh run view --log-failed` 查證，發現 2026-08-27T09:30:17Z 的 `Daily Stock Analysis` workflow_dispatch 執行在「Refresh market and news data」這個 step 卡了 12 分鐘逾時失敗，整個 job 沒有走到後面「Generate interactive report」「Send LINE report link」步驟，所以當天完全沒有報告也沒有發 LINE。**根因是我上一輪剛加的 HiStock 重試機制**：`fetch_histock_branch_snapshot` 對 degraded 結果重試最多 3 次、每次間隔 4 秒，這個假設是「大部分請求會成功、只有少數需要重試」；但當天 HiStock 剛好整站持續異常，`load_histock_broker_summaries` 的 symbol x date 雙層迴圈裡**每一次**呼叫都 degraded 到底，光是失敗當次的 log 就有 81 次 `degraded_retry` 警告，等於白白多燒 5 分鐘以上的 sleep，把原本能在 12 分鐘內跑完的 step 直接拖到逾時。**修正**：在 `load_histock_broker_summaries`(`chip_snapshot.py`)加斷路器——連續 `HISTOCK_BROKER_OUTAGE_CIRCUIT_BREAKER_THRESHOLD`(3)次「重試到用盡都還是 degraded」就判斷是整站異常，之後這次執行改成 `max_attempts=1`(不再重試)，把重試的最大額外成本從「隨迴圈次數線性放大」變成「固定上限（3 次 x 2 次重試 x 4 秒 = 24 秒）」。新增 2 個回歸測試(斷路器觸發時序、成功時不觸發斷路器持續正常重試)。230 測試全過、ruff 乾淨。**教訓**：加任何「失敗時重試」的機制，一定要想清楚「如果失敗是系統性、不是偶發的」會發生什麼事——重試次數 x 呼叫次數 x 延遲秒數，在迴圈裡很容易不知不覺超過 CI 的時間預算，必須配一個「連續失敗達到門檻就放棄重試」的斷路器，不能只看單一函式呼叫本身合不合理
- **報告 UI 第七輪:璞玉主軸區塊上移並排 + 滿版 + 表格標題單行 + HiStock 重試(2026-08-27)**:回應使用者四點回饋——(1)「璞玉選股名單、選股優先順序表、互動技術分析往上搬到璞玉指數動向區塊下」：`hybrid.py` 的 `_save_report` 重排，這三塊現在緊接在璞玉指數動向之後，資金/籌碼/產業面（融資餘額/外資期貨/外資動向/RSS/產業鏈）往後移。(2)「璞玉選股名單、選股優先順序表並排呈現」：`_pristine_watchlist_section` 補上 `<article class="report-card">` 包裹，與選股優先順序表一起包進 `.report-grid--two`。(3)「頁面滿版呈現」：`report_markdown.py` 的 `main` 容器從固定 `max-width:1040px` 改成 `width:96%` + `max-width:1600px` + `box-sizing:border-box`（一開始試過 `max-width:min(1600px,96vw)`，實測在這個瀏覽器環境下 `min()`+`vw` 組合會讓版面寬度不穩定飄動，改用純 `width:96%` 才穩定，且用 `box-sizing:border-box` 避免 padding 造成溢出）。(4)「表格標題盡量一行」：`th` 加 `white-space:nowrap`。**HiStock 重試**：`broker_source.py` 的 `fetch_histock_branch_snapshot` 新增對 `degraded`（sentinel 空版型）結果的重試機制，最多 3 次、每次間隔 4 秒並清掉該次快取。已用真實資料端到端驗證版面順序/並排/滿版(1600px下內容區1521px置中無橫向捲軸、K線圖canvas放大到1128px)/表格標題單行；重試機制也用真實網路測過會正確觸發，但**誠實記錄一個發現**：這次 HiStock 的異常是「持續一段時間」而非「單次隨機」，3 次重試在幾秒內全部拿到同一份 sentinel 回應，代表重試對這種「網站整體卡住」的情境幫不上忙，只對「偶發、單次請求路由到壞節點」的情境有效——沒有進一步加大重試次數/延遲，因為那樣做只會拖慢 pipeline 卻不保證真的解決。228 測試全過、ruff 乾淨
- **前十大主力券商過長 + 順帶挖出券商名稱被黏在一起的真 bug(2026-08-27)**:使用者反映「前十大主力券商」欄位太長，先做顯示層截斷(`report_technical_chart_js.py` 新增 `formatBrokerList()`，超過3家改顯示「前3家 等N家」)。截斷後檢查真實資料時發現**券商名稱本身就是亂的**——例如「群益金鼎-內湖39939973.67美林6171-16573.14富邦-台北」這種數字跟券商名黏在一起的字串，不是單純截斷不夠短的問題。追查到 `broker_source.py` 的 `_ROW_RE` 正則：買/賣張欄位用 `[\d,]+`(要求至少一位數字)，但真實頁面裡一檔券商當天若只出現在買方或賣方，另一側儲存格是完全空的 `<td></td>`，導致該列配對失敗，`finditer` 只好從下一列重新配對，結果把好幾列的券商名稱黏成一長串。實測 2388.TW 某日資料，修正前只配對出 6 筆(應有15筆)且名稱黏連，修正後(`[\d,]+`→`[\d,]*`)正確配對出 15 筆買方+15筆賣方，每筆券商名稱乾淨獨立。新增回歸測試涵蓋空儲存格情境。已用真實資料端到端驗證：修正後「前十大主力券商」正確顯示「凱基-台北、美商高盛、新加坡商瑞銀 等 10 家」。ruff/226 測試全過
- **修正對 HiStock 分點資料的錯誤診斷 + 真正修好籌碼快照 n/a(2026-08-27)**:上一輪(2026-08-26)診斷「HiStock 分點頁面已改版成前端JS動態渲染，scraper 永久失效」**是錯的**，用真實瀏覽器逐步比對後發現：網站沒有改版，資料照樣內嵌在 HTML 裡（`var jsonDatas = eval({...})`，且舊式 `<tr><td><a href=".../brokertrace.aspx...">` 表格列也同時存在），我方 `_ROW_RE` 正則解析完全正常。**真正原因是 HiStock 分點買賣資料有約 2 個交易日的公告延遲**：實測同一天與前一天查詢一律回傳空表(`jsonDatas.Buy/Sell` 皆為 `[]`)，但 2 個交易日前的日期就有完整真實資料，我方 `fetch_histock_branch_snapshot` 對已公告的日期完全能正確解析出 12~13 筆真實分點交易。舊code的 `_enrich_report_chip_snapshots` 固定抓「report_date 往前數 3 個交易日」，這個區間**永遠**落在公告延遲之內，注定每天都撲空。修正：新增 `HISTOCK_BROKER_PUBLISH_LAG_SESSIONS = 2`，改抓 5 個交易日再跳過最近 2 天，只用「已公告的 3 天」去查分點。已用真實資料驗證：修正前 `chip_data_source_status` 全部是 `official-only`，修正後變成 `official+broker|rank=...`，且有真實的主力淨買超數字與分點名稱。ruff/225 測試全過。**教訓**：上次沒有充分交叉比對「同一次請求换一個已知會有資料的舊日期」就下了「網站改版」的結論，這次靠使用者要求「確定」才重新徹底查證，以後遇到外部資料源異常應先排除「資料本來就還沒公布」這個更常見的可能性，再懷疑網站改版
