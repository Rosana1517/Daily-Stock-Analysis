...
已完成(璞玉整合輪):
- **切片 P8「產業鏈上中下游群體共識判讀」完成**:新模組 `stock_signal_system/data/industry_chain.py`,爬 `ic.tpex.org.tw`(產業價值鏈資訊平台)`introduce.php?ic={產業代碼}` 頁面,`parse_industry_chain_html` 純函式從內嵌 HTML 解析出上/中/下游分層與各層成分股,`build_industry_chain_index` 對 39 個產業代碼建立 symbol→歸屬 的長 TTL(30天)快取索引,`find_chain_consensus_groups` 純函式判定共識。**判讀規則經 AskUserQuestion 與使用者確認**(三題皆選推薦選項):範圍限候選池內個股(零額外網路請求)、訊號=★最佳買點/☆短線買點/◆超跌抄底任一、共識門檻=同層≥2檔同步。`hybrid.py` 新增「產業鏈同步訊號」報告區塊,接在外資期貨未平倉之後。**開發過程中用真實資料抓出一個 bug 並修正**:實測發現同一公司(如聯發科)在同一頁同一子分類下重複出現達 10 次(來源網站的渲染瑕疵,非真實資料),原始解析器會照單全收造成重複計數,修正為依 (子分類代碼, 股票代號) 去重後,聯發科正確變回只出現 1 次,三個實測產業(通信網路/半導體/電腦及週邊設備)成分股數從 552/1032/1298 校正為合理的 276/442/649。13 個新測試(HTML 解析含三層/兩層/br標籤清理/去重情境 + 共識分組含達標/未達標/跨層不合併/資料缺失情境),219 測試全過、ruff 乾淨,已用真實資料端到端驗證解析與去重正確
- **P8 資料源查證(2026-08-26)**:找到官方公開網站 `ic.tpex.org.tw`「產業價值鏈資訊平台」(證券櫃檯買賣中心+臺灣證券交易所共同建置),涵蓋約 50 個產業鏈分類,結構為傳統伺服器端渲染 PHP、無文件化 API 但資料直接內嵌在 HTML(`introduce.php?ic={產業代碼}`),用 `chain-title-panel`(上游/中游/下游)+ `ic_link_{子分類代碼}` + `companyList_{子分類代碼}`(內含 `stk_code=` 個股代號)三層結構清楚標示每檔個股在產業鏈中的位置,已用瀏覽器實測通信網路(I000)頁面確認資料真實可爬(抓到敦吉2459/亞信3169/宇智6470等)。**解除了原本記錄的「待使用者提供產業鏈對應表」阻塞**——官方本身就有現成可爬的對照表。詳細技術結構與採集方法設計已寫入 PRD.md「P8 資料源查證結果」。判讀規則(同產業鏈上中下游訊號如何算「群體共識」)與是否要實際動工,仍待使用者決定,本次僅完成研究、未寫程式碼
