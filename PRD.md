...
### 2026-07 新增功能(本輪開發)
- [ ] **★最佳買點標籤**:在「發動確認(原新版策略)」既有入選條件之外,額外標記同時滿足「收盤價站上 60 日均線 + MACD 剛形成黃金交叉(DIF 於最近 2 個交易日內向上穿越訊號線)」的股票為 ★最佳買點。**只加標籤不加篩選**——不改變入選名單,但在報告排序與 LINE 推播中優先顯示
- [ ] **大盤外資動向**:每日報告新增「外資動向」區塊,彙總 TWSE 三大法人(T86)資料,顯示外資近 10 個交易日對台股整體的每日買賣超(張)、連買/連賣天數,並給出偏多/偏空/中性的態度判讀
- [ ] **策略漏斗式重命名**:三層策略改為角色導向命名並依使用順序排列——舊版策略→「品質底池(選股範圍)」、籌碼雷達→「主力動向(誰在買)」、新版策略→「發動確認(何時買)」,★最佳買點為頂層;報告與互動篩選介面依此重排。內部程式碼欄位名(`legacy_hit`/`chip_radar_hit`/`new_strategy_hit`)不變,僅改使用者可見文案
